...
pub const struct_ws_server = opaque {};
pub const ws_server = struct_ws_server;
pub const struct_ws_header = extern struct {
    name: [*c]const u8 = @import("std").mem.zeroes([*c]const u8),
    value: [*c]const u8 = @import("std").mem.zeroes([*c]const u8),
};
pub const ws_header = struct_ws_header;
pub const struct_ws_request = extern struct {
    fd: c_int = @import("std").mem.zeroes(c_int),
    method: [*c]const u8 = @import("std").mem.zeroes([*c]const u8),
    path: [*c]const u8 = @import("std").mem.zeroes([*c]const u8),
    http_major: c_int = @import("std").mem.zeroes(c_int),
    http_minor: c_int = @import("std").mem.zeroes(c_int),
    keep_alive: c_int = @import("std").mem.zeroes(c_int),
    headers: [*c]const ws_header = @import("std").mem.zeroes([*c]const ws_header),
    headers_count: usize = @import("std").mem.zeroes(usize),
    body: [*c]const u8 = @import("std").mem.zeroes([*c]const u8),
    body_len: usize = @import("std").mem.zeroes(usize),
    server: ?*ws_server = @import("std").mem.zeroes(?*ws_server),
};
pub const ws_request = struct_ws_request;
pub const ws_handler_fn = ?*const fn ([*c]const ws_request, ?*anyopaque) callconv(.c) c_int;
pub const WS_LOG_ERROR: c_int = 0;
pub const WS_LOG_WARN: c_int = 1;
pub const WS_LOG_INFO: c_int = 2;
pub const WS_LOG_DEBUG: c_int = 3;
pub const enum_ws_log_level = c_uint;
pub const ws_log_level = enum_ws_log_level;
pub const ws_log_fn = ?*const fn (ws_log_level, [*c]const u8, ?*anyopaque) callconv(.c) void;
pub const WS_QUEUE_BLOCK: c_int = 0;
pub const WS_QUEUE_DROP_NEW: c_int = 1;
pub const WS_QUEUE_DROP_OLDEST: c_int = 2;
pub const enum_ws_queue_policy = c_uint;
pub const ws_queue_policy = enum_ws_queue_policy;
pub const struct_ws_server_opts = extern struct {
    port: c_ushort = @import("std").mem.zeroes(c_ushort),
    nthreads: c_uint = @import("std").mem.zeroes(c_uint),
    queue_capacity: usize = @import("std").mem.zeroes(usize),
    backlog: c_int = @import("std").mem.zeroes(c_int),
    enable_keepalive: c_int = @import("std").mem.zeroes(c_int),
    keepalive_timeout_ms: c_int = @import("std").mem.zeroes(c_int),
    max_keepalive_requests: c_int = @import("std").mem.zeroes(c_int),
    log_fn: ws_log_fn = @import("std").mem.zeroes(ws_log_fn),
    log_user: ?*anyopaque = @import("std").mem.zeroes(?*anyopaque),
    log_level: ws_log_level = @import("std").mem.zeroes(ws_log_level),
    log_format: [*c]const u8 = @import("std").mem.zeroes([*c]const u8),
    log_to_stderr: c_int = @import("std").mem.zeroes(c_int),
    queue_policy: ws_queue_policy = @import("std").mem.zeroes(ws_queue_policy),
};
pub const ws_server_opts = struct_ws_server_opts;
pub const struct_ws_metrics = extern struct {
    accepted: u64 = @import("std").mem.zeroes(u64),
    enqueued: u64 = @import("std").mem.zeroes(u64),
    dequeued: u64 = @import("std").mem.zeroes(u64),
    dropped_new: u64 = @import("std").mem.zeroes(u64),
    dropped_oldest: u64 = @import("std").mem.zeroes(u64),
    bytes_sent: u64 = @import("std").mem.zeroes(u64),
    responses_2xx: u64 = @import("std").mem.zeroes(u64),
    responses_3xx: u64 = @import("std").mem.zeroes(u64),
    responses_4xx: u64 = @import("std").mem.zeroes(u64),
    responses_5xx: u64 = @import("std").mem.zeroes(u64),
    queue_len: usize = @import("std").mem.zeroes(usize),
    queue_max_observed: usize = @import("std").mem.zeroes(usize),
    nthreads: c_uint = @import("std").mem.zeroes(c_uint),
};
pub const ws_metrics = struct_ws_metrics;
pub extern fn ws_server_new(port: c_ushort) ?*ws_server;
pub extern fn ws_server_new_ex(port: c_ushort, nthreads: c_uint) ?*ws_server;
pub extern fn ws_server_new_with_opts(opts: [*c]const ws_server_opts) ?*ws_server;
pub extern fn ws_server_request_shutdown(s: ?*ws_server) c_int;
pub extern fn ws_server_get_metrics(s: ?*ws_server, out: [*c]ws_metrics) c_int;
pub extern fn ws_send_response(req: [*c]const ws_request, status: c_int, reason: [*c]const u8, content_type: [*c]const u8, body: ?*const anyopaque, body_len: usize) isize;
pub extern fn ws_server_add_route(s: ?*ws_server, method: [*c]const u8, path: [*c]const u8, cb: ws_handler_fn, user_data: ?*anyopaque) c_int;
pub extern fn ws_server_run(s: ?*ws_server) c_int;
pub extern fn ws_server_free(s: ?*ws_server) void;
...
pub const __ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__ = @import("std").zig.c_translation.promoteIntLiteral(c_int, 150701, .decimal);
pub const __ENVIRONMENT_OS_VERSION_MIN_REQUIRED__ = @import("std").zig.c_translation.promoteIntLiteral(c_int, 150701, .decimal);
...
pub const __DARWIN_ALIAS_STARTING_MAC___MAC_16_0 = @compileError("unable to translate C expr: unexpected token ''");
// /Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/_symbol_aliasing.h:995:9
pub const __DARWIN_ALIAS_STARTING_MAC___MAC_26_0 = @compileError("unable to translate C expr: unexpected token ''");
// /Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/_symbol_aliasing.h:1001:9
...
pub const WS_MAX_HEADERS = @as(c_int, 255);
